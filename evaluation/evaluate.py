...
from evaluation.rerank import rerank_results
...
def evaluate_retrieval(queries: List[Dict[str, Any]], retrieval_function: Callable, db, k: int = 20) -> Dict[
    str, float]:
    total_score = 0
...
def evaluate_retrieval_rerank(queries: List[Dict[str, Any]], retrieval_function: Callable, db, k: int = 20) -> Dict[
    str, float]:
    total_score = 0
    total_queries = len(queries)
    ...
    for query_item in tqdm(queries, desc="Evaluating retrieval"):
        ...
        retrieved_docs = retrieval_function(query, db, k=k*3)
        retrieved_docs = rerank_results(query, retrieved_docs, k=k)
...
def evaluate_db_advanced(db, original_jsonl_path, k):
    original_data = load_jsonl(original_jsonl_path)
    results = evaluate_retrieval_rerank(original_data, retrieve_base, db, k)
    print(f"Pass@{k}: {results['pass_at_n']:.2f}%")
